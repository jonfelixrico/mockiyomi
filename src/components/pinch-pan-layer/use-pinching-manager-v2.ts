...
export function usePinchingManager(
  ...
  function handlePinch(
     ...
     */
    const pinchAnchor = {
      x: (location.x + scroll.left) / contentDims.width,
      y: (location.y + scroll.top) / contentDims.height,
    }
    ...
    const dimsAfterResize = {
      width: (contentDims.width / scaleToDisplay) * boundedScale,
      height: (contentDims.height / scaleToDisplay) * boundedScale,
    }
